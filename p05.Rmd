---
title: "Using Functions and Loops to Compile Many Regression Analyses"
output: 
  html_document:
    theme: cosmo
    toc: true
...
> The goal of this project is to use functions and loops to run many regression analyses. 

> The data is from a study which, in part, measures the association between religiosity and well-being. The well-being variable was a composite of five different measures of well-being (depression, anxiety, life satisfaction, postive affect, and negative affect). Because the preregistration for the study specified that separate regression analyses would be run for each of the constituent well-being variables, and for the two subscales of the religiosity scale, and again with models containing demographic control variables, the number of regression analyses to run for a single hypothesis about the relationship between religiosity and well-being became burdensome. This was the reason for attempting to create a function to run different versions of a regression model (including and excluding control variables) and a loop which would run the function for each combination of explanatory and outcome variable.
