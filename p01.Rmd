...
library(tidyr)
library(haven)
partisan_bias <- read_sav("p01/data/BIAS_DATA_LONG.sav")
```

Now, that I've imported the data, I'd like to get summary statistics.

```{r summary stats, message=FALSE}
summary_stats <- partisan_bias %>% 
  group_by(delib, partyid, target, time) %>% 
  summarise(
